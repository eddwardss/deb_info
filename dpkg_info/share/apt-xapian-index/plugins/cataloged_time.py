...
class CatalogedTime:
    ...
    def __init__(self, **kwargs):
        ...
        self._packages_cataloged_file = os.path.join(self.PERSISTANCE_DIR, self.CATALOGED_NAME)

        if os.path.exists(self._packages_cataloged_file):
            with open(self._packages_cataloged_file, "rb") as f:
                self._package_cataloged_time = pickle.load(f)
        else:
...
#    def init(self, info, progress):
        if os.access(self.PERSISTANCE_DIR, os.W_OK):
            ...
            self._packages_cataloged_file = None
        if self._packages_cataloged_file and os.path.exists(self._packages_cataloged_file):
            with open(self._packages_cataloged_file, "rb") as f:
                self._package_cataloged_time = pickle.load(f)
        else:
            self._package_cataloged_time = {}
...
#        document.add_value(self.value, xapian.sortable_serialise(time))
        ...
        time_val = self._package_cataloged_time.get(pkg.name, self.now)
        self._package_cataloged_time[pkg.name] = time_val
        if self.value >= 0:
            document.add_value(self.value, xapian.sortable_serialise(int(time_val)))
